...
extern "C" UINT __stdcall Install(MSIHANDLE hInstall)
{
	...
	try {
		...
		//$ServiceName=[EDITB1]$AgentPort=[EDITB2]$Ips=[EDITA1]$Target=[TARGETDIR]$Devices=[EDITA2]
		...
		SetFilePermission(path+"FanucAgent.exe") ;
		SetFilePermission(path+"InstallAgent.bat") ;
		SetFilePermission(path+"MTCFanucAgent.ini") ;
		...
		*/
		...
		_xmlconfig.WriteAgentCfgFile("Agent.cfg", "Devices.xml", path, httpPort,	empty, empty,  empty, ServiceName);
		
		WritePrivateProfileString("CONFIG", "HttpPort", httpPort.c_str(), (path+"MTCFanucAgent.ini").c_str());
