...
static int GetShiftTime (std::string s)
{
    ...
    {
        throw std::exception("Bad Shift time format -  need hh:mm\n");
    }
...
int AgentConfigurationEx::thread (int aArgc, const char *aArgv[])
{
    ...
    try
    {
        ...
        if ( GetFileAttributesA(cfgfile.c_str( ) ) != INVALID_FILE_ATTRIBUTES )
        {
            ...
            _agentcfg.names  = TrimmedTokenize(config.GetSymbolValue("CONFIG.FanucDevices", "Fanuc1").c_str( ), ",");

            if ( Globals.fanucips.size( ) != _agentcfg.names.size( ) )
            {
                throw std::string("Mismatched number of ips and device names i.e., - FanucIpAddress &FanucDevices ");
            }
            ...
            Globals.sNewConfig = config.GetSymbolValue("CONFIG.NewDevicesFile", "OLD").c_str( );
        ...
        if ( Globals.sNewConfig == "NEW" )
        {
            try
            {
                ...
                WritePrivateProfileString("CONFIG", "Config", "UPDATED", cfgfile.c_str( ) );
            }
            catch ( ... )
            {
                WritePrivateProfileString("CONFIG", "Config", "ERROR", cfgfile.c_str( ) );
            }
        }
        ...
        for ( int i = 0; i < Globals.fanucips.size( ); i++ )
        {
            ...
            _agentcfg.AddFanucDevice(_agentcfg.names[i], config); // changed 3/29/2013
        }
    ...
    {
        GLogger << FATAL << "MTConnect Agent Service Configuration Failed " << nowtimestamp( ) << errmsg.c_str( ) << "\n";
        return -1;
    }
    catch ( ... )
    {
