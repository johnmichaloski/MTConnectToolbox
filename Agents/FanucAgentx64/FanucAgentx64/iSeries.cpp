	...
	*/
	...
	if (ret == EW_OK)
	{
		...
		try {
			...
			{
				GLogger.Debug(StrFormat( "Axis: %s Data: %ld Dec: %h \n" , PosData[i].abs.name, PosData[i].abs.data, PosData[i].abs.dec));
	...
	try {
		...
		for(int i=3; i< _nAxes; i++)
		{
			...
			//GLogger.LogMessage(StrFormat("CiSeries::getPositions cnc_rdsvmeter Axis %c Data=%d Dec=%d\n", sv[i].svload.name, sv[i].svload.data, sv[i].svload.dec), GLogger.HEAVYDEBUG);
...
int CiSeries::connect()
{
	...
	{
		GLogger.LogMessage(StrFormat("iSeries::connect HSSB cnc_allclibhndl2 Port=%d\n", _adapter->mDevicePort ), FATAL);
		//GLogger.Fatal(StrFormat("THIS EXECUTABLE DOES NOT HANDLE HSSB - EXCLUSIVELY Focas w/ Processing library for TCP/IP\n"));
#ifdef iSERIESHSSB
		ret =  ::cnc_allclibhndl2(_adapter->mDevicePort,&_adapter->mFlibhndl);
#else
		GLogger.Fatal(StrFormat("THIS EXECUTABLE DOES NOT HANDLE LAN - EXCLUSIVELY Focas w/ Processing library for HSSB\n" ));
		return -1;
#endif
	}
	else
	{
		...
		GLogger.LogMessage(StrFormat("iSeries::connect LAN cnc_allclibhndl3 IP=%s  Port=%d\n",_adapter->mDeviceIP.c_str(), _adapter->mDevicePort ), FATAL);
#ifdef iSERIESLAN
	...
	GLogger.Fatal(StrFormat("iSeries::connect cnc_allclibhndl Result: %h\n" , ret ));
	if (ret != EW_OK) 
	{
