...
{
	std::string errmsg =  StrFormat("COpcAdapter In trans_func - Code = 0x%x\n",  pExp->ExceptionRecord->ExceptionCode);
...
void CCmdHandler::Configure(nist::Config	& config,
							std::string device, 
							std::string ipaddr,
							std::string port
							)
{
	...
	heartbeat=0;
	std::string sHeartbeat=StrFormat("%d",heartbeat++);
...
{
	GLogger.Fatal(StrFormat("CCmdHandler::Cycle()\n" )); 
	HRESULT hr;
	int nHeartbeat=0;
	_mRunning=true;
	...
	while(_mRunning)
	{
		...
		{
			GLogger.Info(StrFormat("CCmdHandler::Cycle() exception %s\n" , e.what() ));
		}
		catch(...)
		{
			GLogger.Info(StrFormat("CCmdHandler::Cycle() exception \n"));
		}
	}
	::CoUninitialize();
}
