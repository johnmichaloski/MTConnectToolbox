...
extern "C" UINT __stdcall Install(MSIHANDLE hInstall)
{
	...
	std::string path,agentport,devices,servicename,UNCFiles, user, pw;
	std::string status;
	std::string ServiceName = "AgfmAgent";

	try {

		// Asssign defaults
		devices="M1";
		servicename="AgfmAgent";
		agentport="5000";
		...
		for(int i=0 ; i< symbols.size() ; i++)
		{
			...
			// Ini file parameters
			if(tokens[0]=="AgentPort") // no change necessary if using csv, program will stip out
				agentport=tokens[1];
			...
			if(tokens[0]=="UNCFileName") // no change necessary if using csv, program will stip out
				UNCFiles=tokens[1];    
			if(tokens[0]=="User") // no change necessary if using csv, program will stip out
				user=tokens[1];    
			if(tokens[0]=="Password") // no change necessary if using csv, program will stip out
				pw=tokens[1];    
		}
		...
		SetFilePermission(path+"AgfmAgent.exe") ;
		...
		ChangeIniParameter(path+"Config.ini", "ServiceName", servicename); //ServiceName=AgfmAgent
		...
		std::vector<std::string> values = TrimmedTokenize(UNCFiles, ",");
...
extern "C" UINT __stdcall Uninstall (MSIHANDLE hInstall)
{
	...
	*/
	...
	try{
		std::string str;
		//RunSynchronousProcess("sc.exe", " stop AgfmAgent");
		//::Sleep(3000);
		RunSynchronousProcess("sc.exe", " delete AgfmAgent");
	}
	catch(...)
	{
