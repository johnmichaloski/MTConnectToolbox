	...
	std::vector<std::string> & conditionIds)
{
	...
	if(find(tagnames.begin(), tagnames.end(), "version") != tagnames.end())
		tmp+="\t\t\t\t<DataItem id=\"####version\" name=\"version\"  type=\"MESSAGE\" category=\"EVENT\"/>\n";
	if(find(tagnames.begin(), tagnames.end(), "motherboard_temp") != tagnames.end())
		tmp+="\t\t\t\t<DataItem id=\"####motherboard_temp\" name=\"motherboard_temp\" type=\"TEMPERATURE\" category=\"SAMPLE\"/>\n";
	...
	for ( size_t i = 0; i < jointnames.size( ); i++ )
	{
		...
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_actpos") != tagnames.end())
		...
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_mode") != tagnames.end())
			tmp += StdStringFormat("\t\t\t\t\t\t\t\t<DataItem type=\"MESSAGE\"  "
			"id=\"####_%s_mode_id\" category=\"EVENT\" "
			"name=\"%s_mode\" />\n",
			jointnames[i].c_str( ), jointnames[i].c_str( ));
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_I") != tagnames.end())
			tmp
			+= StdStringFormat("\t\t\t\t\t\t\t\t<DataItem type=\"ELECTRICAL_ENERGY\"  "
			"id=\"####_%s_I_id\" category=\"SAMPLE\" name=\"%s_I\" />\n",
			jointnames[i].c_str( ), jointnames[i].c_str( ));
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_V") != tagnames.end())
			tmp
			+= StdStringFormat("\t\t\t\t\t\t\t\t<DataItem type=\"VOLTAGE\"  "
			"id=\"####_%s_V_id\" category=\"SAMPLE\" name=\"%s_V\" />\n",
			jointnames[i].c_str( ), jointnames[i].c_str( ));
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_T") != tagnames.end())
			tmp
			+= StdStringFormat("\t\t\t\t\t\t\t\t<DataItem type=\"TORQUE\"  "
			"id=\"####_%s_T_id\" category=\"SAMPLE\" name=\"%s_T\" />\n",
			jointnames[i].c_str( ), jointnames[i].c_str( ));
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_actvel") != tagnames.end())
		...
		if(find(tagnames.begin(), tagnames.end(),jointnames[0] + "_actacc") != tagnames.end())
			tmp +=StdStringFormat(
	...
	if(find(tagnames.begin(), tagnames.end(),"pose") != tagnames.end())
		"\t\t\t\t\t\t\t\t<DataItem type=\"PATH_POSITION\" id=\"####pp\" "
		"category=\"SAMPLE\" name=\"pose\" subType=\"ACTUAL\" />\n";

	if(find(tagnames.begin(), tagnames.end(),"velocity") != tagnames.end())
		"\t\t\t\t\t\t\t\t<DataItem type=\"PATH_FEEDRATE\" id=\"####pfr\" "
		"category=\"SAMPLE\" name=\"velocity\" subType=\"ACTUAL\" />\n";

	if(find(tagnames.begin(), tagnames.end(),"program") != tagnames.end())
		tmp+="\t\t\t\t\t\t\t\t<DataItem type=\"PROGRAM\" id=\"####pgm\" "
		"category=\"EVENT\" name=\"program\"/>\n";

	if(find(tagnames.begin(), tagnames.end(),"execution") != tagnames.end())
		tmp+="\t\t\t\t\t\t\t\t<DataItem type=\"EXECUTION\" id=\"####exec\" "
		"category=\"EVENT\" name=\"execution\"/>\n";

	if(find(tagnames.begin(), tagnames.end(),"robotmode") != tagnames.end())
		tmp+="\t\t\t\t\t\t\t\t<DataItem type=\"CONTROLLER_MODE\" "
		"id=\"####robotmode\" category=\"EVENT\" name=\"robotmode\"/>\n";

	if(find(tagnames.begin(), tagnames.end(),"controlmode") != tagnames.end())
		tmp+="\t\t\t\t\t\t\t\t<DataItem type=\"CONTROLLER_MODE\" "
		"id=\"####controlmode\" category=\"EVENT\" name=\"controlmode\"/>\n";

	if(find(tagnames.begin(), tagnames.end(),"fovr") != tagnames.end())
		tmp+="\t\t\t\t\t\t\t\t<DataItem type=\"PATH_FEEDRATE\" id=\"####pfo1\" "
		"category=\"SAMPLE\" name=\"fovr\" units=\"PERCENT\" "
		"nativeUnits=\"PERCENT\" subType=\"OVERRIDE\" />\n";

	if(find(tagnames.begin(), tagnames.end(),"frt") != tagnames.end())
		tmp+="\t\t\t\t\t\t\t\t<DataItem type=\"PATH_FEEDRATE\" id=\"####pfr1\" "
		"category=\"SAMPLE\" name=\"frt\" />\n";

	tmp+="\t\t\t\t\t\t\t\t<DataItem category=\"EVENT\"  id=\"####heartbeat\"  "
