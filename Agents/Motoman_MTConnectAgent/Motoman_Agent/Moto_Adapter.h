...
public:
	...
	*/
	...
	void setMTCConditionValue (std::string tag, std::string value);
	...
	*/
	...
	/**
	* @brief isValidCondition returns true if MTConnect conditional value is valid
	* @param val string value of condition
	* @return true valie.
	*/
	bool isValidCondition(std::string val);

	/**
	* @brief doStop corresponds to MTC Institute agent stop method.Stops this
	* adapter. Each different variety of adapter typically has a different
	* method and length of time to stop.
	*/
	virtual void doStop ( );
	...
	*/
	...
	{
		/**
		* @brief mAlias device tagname as opposed to MTConnect tagname
	...
	*/
	...
	std::string mDevice;                                   //!<  adapter name for device (containing no spaces)
	...
	std::vector<std::string>  mConditionTags;               //!< list of tagnames that are conditions
};

class Moto_Adapter : public AdapterT
{
public:
	Moto_Adapter(AgentConfigurationEx *mtcagent,
		Nist::Config &        config,
		std::string           device)
	...
	*/
	Item *          createItem (std::string tag, std::string type = "Event");
	...
	*/
	...
	std::string mMotodfFile;                               //!< filename path of urdf
	float mMotoVersion;                                    //!< version of UR communication interface from ini
	std::string mPort;                                     //!< status tcp/ip port of the ur robot controller.

	// UR message handling - use mutex queue for communication
	typedef std::vector<uint8_t> msgbuffer_t;                //!< type of ur message vector
	RCS::CMutexBuffer mMotoBuffer;                          //!<  mutexed message queue
	moto_communication mMotoComm;                           //!< ur communication handler
	moto::moto_robot_state mMotoData;                       //!< ur message and data decoding handler
	moto::moto_robot_state mMotoDataCopy;                       //!< copy current ur message and data decoding handler
