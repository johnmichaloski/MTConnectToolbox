...
public:
	...
	virtual int ReadOPCSettings(std::string filename);  // done 1st
	...
	virtual void ErrorMessage(std::string errmsg) { GLogger.Error(errmsg); }
	virtual void DebugMessage(std::string errmsg) { GLogger.Warning(errmsg); }
	std::string  & TagSectionName() { return _sTagSectionName; }
	...
	virtual std::string GetSymbolValue(std::string section, std::string tag, const char * defaultval) =0;
	virtual std::vector<std::string>  GetSectionKeys(std::string section)=0;

	template <typename T>
	T GetSymbolValue(std::string section, std::string tag, T defaultval)
	{ 
		return ConvertString<T>(GetSymbolValue(section, 
			tag,
...
protected:
	...
	std::string _CNCTaskListCmd;
	std::string _gServerProgId;

	//std::string _gInifilename;
	//std::string _contents;
	std::string _sTagSectionName;
	std::string _gszTaskListCmd;
	std::string _gszStartupErrors;
	//std::string _szLoggingFilename;
	int _nPingTimeout;

	// OPC Server parameters
	std::string _gszCncControllerProcessName;
	std::string _opcservermachine;
	std::string _gszOPCServerName;
	std::string _sClsid;
};
...
HRESULT COPCBackEnd<TOPCItem>::Connect() 
{
	...
	DebugMessage(StdStringFormat("%s Fcn Machine %s \n",name, _opcservermachine.c_str() ));
	...
	try 
	{
		...
		_opcservermachine=MakeLower(_opcservermachine);

		if(!_opcservermachine.empty())
			DebugMessage(StdStringFormat("%s Machine %s - TestPing %d\n",name, _opcservermachine.c_str(),_nPingTimeout ));
		if((!_opcservermachine.empty()) && _nPingTimeout >0 && Ping(_opcservermachine.c_str(), _nPingTimeout))
		{
			::Sleep(20*1000);
			throw StdStringFormat( _T("%s WARNING: Couldn't ping machine=%s"), name, _opcservermachine.c_str());
		}


		if(_opcservermachine.size() < 1)
		{
			DebugMessage(StdStringFormat("%s Machine %s - Local CoCreateInstance\n",name, _opcservermachine.c_str() ));
			// create instance of the OPC server 
			if(!_gServerProgId.empty())
			{
				hResult= CLSIDFromProgID( _bstr_t(_gServerProgId.c_str()),  &_gOpcServerClsid);
				 
				if(hResult==CO_E_CLASSSTRING) 
					throw std::exception(StdStringFcn("%s The registered CLSID for the ProgID is invalid. %s\n",name, _gServerProgId.c_str()));
				if(hResult==REGDB_E_WRITEREGDB) 
					throw std::exception(StdStringFcn("%s An error occurred writing the CLSID to the registry.\n", name));
				if(FAILED(hResult))
					throw std::exception(StdStringFcn("%s An error occurred retrieving CLSID from _gServerProgId.\n", name, _gServerProgId.c_str()));
			...
			{
				_pIOPCServer=NULL;
				DebugMessage(StdStringFormat("%s Machine %s - Couldn't create CoCreateInstance %x\n", name, _opcservermachine.c_str(),hResult ));
				throw StdStringFormat( _T("%s WARNING: Couldn't create %s-Server\n CLSID=%s Error=%x"), name, _gszOPCServerName.c_str(), _sClsid.c_str(), hResult);
			}
		}
		else {
...
#if 1
			std::vector<std::string> id=Tokenize(GetSymbolValue("OPCSERVER", "User", ""),",") ;
			...
			{
				std::string _domain = (id.size() > 1)? id[0] : ".";
				std::string _user = (id.size() > 2)? id[1] : "auduser";
				std::string _password = (id.size() > 3)? id[2] : "SUNRISE";
				Auth_Identity idn(A2W( _domain.c_str()),A2W(_user.c_str()),A2W(_password.c_str()));
				pAuthIdentityData=idn.GetNullAuth_Identity();
			}

			std::string s_dwAuthzSvc=GetSymbolValue("OPCSERVER", "AuthzSvc", "RPC_C_AUTHZ_NAME") ;
			std::string s_dwAuthnLevel=GetSymbolValue("OPCSERVER", "AuthnLevel", "RPC_C_AUTHN_LEVEL_NONE") ;
			...
			DebugMessage(StdStringFormat("%s Machine %s - Remote CoCreateInstanceEx\n", name, _opcservermachine.c_str() ));
			...
			if (FAILED(hResult))
			{
				DebugMessage(StdStringFormat("%s Machine %s - Remote CoCreateInstanceEx hResult Error %x\n",name, _opcservermachine.c_str(), hResult ));
				throw StdStringFormat (_T("%s FAIL: Remote CoCreateInstanceEx.\n"), name);
			}

			if (FAILED(mqi[0].hr)) 
			{
				DebugMessage(StdStringFormat("%s Machine %s - CoCreateInstanceEx mqi[0].hr Error %x\n",name, _opcservermachine.c_str(), mqi[0].hr ));
				throw StdStringFormat(_T("%s FAIL Remote mqi[0].hr.\n"), name);
			}
		...
		if(_pIOPCServer==NULL)
		{
			DebugMessage(StdStringFormat("%s Machine %s - CoCreateInstanceEx pointer NULL error %x\n",name, _opcservermachine.c_str(),hResult ));
			return E_FAIL;
		}

		// This is how we will detect if the server is connected to the CNC

		DebugMessage(StdStringFormat("%s Machine %s - GetStatus\n", name, _opcservermachine.c_str() ));
		OPCSERVERSTATUS * pServerStatus=NULL;
		if(FAILED(hResult=_pIOPCServer->GetStatus(&pServerStatus)))
		{
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - GetStatus error %x\n",_opcservermachine.c_str(),hResult ));
			//if(pServerStatus!=NULL) 
			//	switch(pServerStatus->dwServerState)
			//{
			//	case OPC_STATUS_RUNNING	: GLogger << INFO  <<  "The server is running normally. This is the usual state for a server"; break;
			//	case OPC_STATUS_FAILED	: GLogger << INFO  <<  "A vendor specific fatal error has occurred within the server. The server is no longer functioning. The recovery procedure from this situation is vendor specific. An error code of E_FAIL should generally be returned from any other server method. "; break;
			//	case OPC_STATUS_NOCONFIG	: GLogger << INFO  <<  "The server is running but has no configuration information loaded and thus cannot function normally. Note this state implies that the server needs configuration information in order to function. Servers which do not require configuration information should not return this state."; break;
			//	case OPC_STATUS_SUSPENDED	: GLogger << INFO  <<  "The server has been temporarily suspended via some vendor specific method and is not getting or sending data. Note that Quality will be returned as OPC_QUALITY_OUT_OF_SERVICE."; break;
			//	case OPC_STATUS_TEST	: GLogger << INFO  <<  "The server is in Test Mode. The outputs are disconnected from the real hardware but the server will otherwise behave normally. Inputs may be real or may be simulated depending on the vendor implementation. Quality will generally be returned normally."; break;
			//}
		...
		DebugMessage(StdStringFormat("%s Machine %s - AddGroup\n", name, _opcservermachine.c_str() ));
		...
		{
			_pIOPCItemMgt=NULL;
			DebugMessage(StdStringFormat("%s Machine %s - AddGroup couldn't add group to server %x\n",name, _opcservermachine.c_str(),hResult ));
			throw std::string (_T("%s FAIL:  couldn't add group to server!\n"),name);
		}


		// Get interface for read operations
		DebugMessage(StdStringFormat("%s Machine %s - QueryInterface IID_IOPCSyncIO\n", name, _opcservermachine.c_str() ));
		if ( FAILED(hResult =  _pIOPCItemMgt->QueryInterface(IID_IOPCSyncIO,(void**)&_pIOPCSyncIO )))
		{
			_pIOPCSyncIO=NULL;
			DebugMessage(StdStringFormat("%s Machine %s - couldn't cculdn't query IOPCSyncIO interface %x\n",name, _opcservermachine.c_str(),hResult ));
			throw StdStringFormat(_T("%s FAIL: cculdn't query IOPCSyncIO interface.\n"),name);
		}

		/////////////////////////////////////////////////////////////////////////////
		// Add items to group
		/////////////////////////////////////////////////////////////////////////////
		DebugMessage(StdStringFormat("%s Machine %s - Add items to group\n",name, _opcservermachine.c_str() ));
		...
		for(j=0,i=0; i< opcitems.size(); i++)
		{
			...
			if(opcitems[i]->vt== VT_USERDEFINED)
				continue;
			DebugMessage(StdStringFormat("%s Machine %s - Add item %s\n",name, _opcservermachine.c_str(),opcitems[i]->_tagname.c_str() ));
			if(FAILED(hResult = AddOPCItem(opcitems[i])))
			{
				throw StdStringFormat(_T("%s  FAIL: Connect() couldn't add item %s to OPC Group.\n"), name, opcitems[i]->_alias.c_str());
		...
		{
			std::string err=StdStringFormat(_T("%s FAIL:  couldn't add any Group items to OPC Server %s.\n"), name, _opcservermachine.c_str());
			DebugMessage(StdStringFormat("%s Machine %s - Add item error %s\n", name, _opcservermachine.c_str(), err.c_str() ));
		}

	}
	catch(std::string errmsg)
	{
		std::string hrerr=StdStringFormat(_T("%s FAIL: OPC Server %s Connect failed %s COM Error=%x \n"),  name, _opcservermachine.c_str(), errmsg.c_str(), hResult);
		ErrorMessage( hrerr);
	...
	::Sleep(_nOPCServerRate);
	DebugMessage(StdStringFormat("%s Machine %s - Exit\n", name, _opcservermachine.c_str()));
	return S_OK;
}
...
HRESULT COPCBackEnd<TOPCItem>::AddOPCItem(COPCItem * item) 
{
	...
	try 
	{
		...
		if(FAILED(hr))
		{
			std::string hrerr=StdStringFormat(_T("%s FAIL: item %s COM Error=%x \n"),  name, item->_tagname.c_str(), hr);
			ErrorMessage( hrerr);
		}
		else if(hr==S_FALSE)
		{
	...
	{
		std::string hrerr=StdStringFormat(_T("%s FAIL: failed \n"),  name);
		ErrorMessage( hrerr);
	}
...
{	
	...
	try
	{
		if (_pIOPCSyncIO==NULL)
			throw std::string(_T("%s Error: Bad IOpSyncIO \n"), name);

		if(opcitems.size()==0)
			throw std::string(_T("%S Error: _hServerHandleItems.size()==0 \n"), name);
		...
		phServers = (OPCHANDLE *) CoTaskMemAlloc (n * sizeof (OPCHANDLE));
		...
		for(int j=0, i=0; i< opcitems.size(); i++)
		{
...
#endif
			{
				...
				opcitems[i]->_value= _variant_t(pItemValues[j].vDataValue); // .Attach(pItemValues[j].vDataValue);
				opcitems[i]->_time=pItemValues[j].ftTimeStamp;
				_CriticalSection.Unlock();
			}
			//if(pItemValues[j].vDataValue.vt == VT_BSTR)
			//	::SysFreeString( pItemValues[j].vDataValue.bstrVal );
	...
	{
		std::string hrerr=StdStringFormat(_T("%s FAIL:  failed %S \n"),  name, errmsg.c_str());
		ErrorMessage( hrerr);
		hr= E_FAIL;
	}
	catch(...)
	{
		std::string hrerr=StdStringFormat(_T("%s FAIL:  exception\n"),  name);
		ErrorMessage( hrerr);
		hr=E_FAIL;
	}
...
template <class TOPCItem>
int COPCBackEnd<TOPCItem>::ReadOPCSettings(std::string filename)
{
	...
	_nQueryServerPeriod=  GetSymbolValue("OPCSERVER", "QueryServerPeriod", (long) 15000);
	...
	_nOPCServerRate=  GetSymbolValue("OPCSERVER", "ServerRate",(long) 5000);
	_gszTaskListCmd=  GetSymbolValue("OPCSERVER", "CNCTaskListCmd", "");
	_nPingTimeout=  GetSymbolValue("OPCSERVER", "PingTimeout", (long) 1000);
