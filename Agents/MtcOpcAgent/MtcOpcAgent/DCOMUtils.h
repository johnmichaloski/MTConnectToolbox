...
extern int __cdecl Ping(const char *host, int nTimeout=100) ;
...
inline bool TestPing(std::string host, int timeout=75 /*miliseconds*/)
{
	int bResult=false;  // assume fail
	...
	delete ReplyBuffer;
	if(dwRetVal!=0)
		OutputDebugString(StdStringFormat("Error=%s\n", ::ErrorFormatMessage(dwRetVal).c_str()).c_str());
	return (dwRetVal != 0) ;
...
inline bool TestPing1(std::string host, int timeout=75 /*miliseconds*/)
{
	std::string tmp;
    ...
    address.sin_addr.s_addr = inet_addr(host.c_str()); /* assign the address */
    address.sin_port = htons(7);            /* translate int2port num - 7 is the ping socket - should be open*/	
...
public:
	...
	std::string authn;
	std::string authz;
	std::string RPC_C_IMP_LEVEL;
	std::string RPC_C_AUTHN_LEVEL;
	StringVector authnitems;
	StringVector authzitems;
	StringVector authenlevelitems;
	StringVector implevelitems;
	...
	int Get(int n, StringVector & items) 
	{ 
