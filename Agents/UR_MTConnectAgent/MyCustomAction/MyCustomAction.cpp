...
extern "C" UINT __stdcall Install(MSIHANDLE hInstall)
{
	...
	try {
		...
		SetFilePermission(path+"UR_Agent.exe") ;
		...
		//RunSynchronousProcess(path+"UR_Adapter.exe", " install");
...
extern "C" UINT __stdcall Uninstall (MSIHANDLE hInstall)
{
	...
	*/
	...
	try{
		std::string str;
		//RunSynchronousProcess("sc.exe", " stop UR_Adapter");
		//::Sleep(3000);
		RunSynchronousProcess("sc.exe", " delete UR_Adapter");
	}
	catch(...)
	{
