...
/**
 * @brief The EventLogging class will log errors into the Windows
 * Computer Management - Event Viewer (accessible only to administrators).
 * Useful for logging service errors as these can be hard to track.
 * Works, but can clog event viewer with errors.
 */
class EventLogging
{
...
#define GENERIC_INVALID     ( (DWORD) 0xC0020100L )
    ...
    // DWORD dwEventID=GENERIC_INVALID )// EVENTLOG_WARNING_TYPE
    // EVENTLOG_INFORMATION_TYPE
    {
        ...
        // This event includes user-defined data as part of the event. The event
        // message
        // does not use insert strings.
        dwEventDataSize = ( (DWORD) wcslen(pInsertStrings[0]) + 1 ) * sizeof( WCHAR );

        if ( !::ReportEvent(hEventLog, EVENTLOG_SUCCESS, 0, 0, NULL, 1, 0,
                            &pAInsertStrings[0], NULL) )

        // if (!ReportEvent(hEventLog, wType, wCategory, 0, NULL, 0,
        // dwEventDataSize, NULL, pInsertStrings[0]))

        // if (!ReportEvent(hEventLog, wType, wCategory, dwEventID, NULL, 0,
        // dwEventDataSize, NULL, pInsertStrings[0]))
        {
