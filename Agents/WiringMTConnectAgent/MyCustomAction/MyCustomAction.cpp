	...
	try {
		...
		for(size_t i=0 ; i< symbols.size() ; i++)
		{
			...
			if(tokens[0]=="AgentPort") // http://agent:portno
				agentport=tokens[1];
			if(tokens[0]=="ServiceName") // default WiringAgent
		...
		SetFilePermission(path+"WiringAgent.exe") ;
		...
		for(size_t i=0; i< keys.size(); i++)
		{
			WritePrivateProfileString(keys[i].c_str(), "NetworkShare", logs[i].c_str(), (path+"Config.ini").c_str());

		}
		DbgOut("Create service with WiringAgent.exe");
		...
		//RunSynchronousProcess(path+"WiringAgent.exe", " install");
		...
		RunSynchronousProcess("sc.exe", StdStringFormat("  create %s binpath= \"%s\"", servicename.c_str(), (path+"WiringAgent.exe").c_str() ));
		//if(!user.empty())
...
extern "C" UINT __stdcall Uninstall (MSIHANDLE hInstall)
{
	...
	*/
	...
	try{
		std::string str;
		//RunSynchronousProcess("sc.exe", " stop WiringAdapter");
		//::Sleep(3000);
		RunSynchronousProcess("sc.exe", " delete WiringAdapter");
	}
	catch(...)
	{
