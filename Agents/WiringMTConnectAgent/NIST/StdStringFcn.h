...
inline std::string extract(std::string s, std::string start, std::string end)
{
	size_t n;
	std::string news;
	if((n=s.find(start))!=std::string::npos)
	{		
		s = Trim(s.substr(n+start.size()));			
		if((n=s.find("]"))!=std::string::npos)
			news= Trim(s.substr(0, n)); 
	}
	return news;
}
