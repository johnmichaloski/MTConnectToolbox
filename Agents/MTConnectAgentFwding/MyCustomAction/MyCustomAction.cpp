...
extern "C" UINT __stdcall Install(MSIHANDLE hInstall)
{
	...
	std::string path,agentport,backendurl,servicename;
	std::string status;

	try {
		Alert(NULL, "Enter custom action", "Alert", MB_OK);
		// Asssign defaults
		servicename="MTConnectAgentFwding";
		agentport="5000";
		...
		status = "Read edit box variables\n";
		// Custom action parameterization
		// $ServiceName=[EDITA1]$AgentPort=[EDITA2]$Devices=[EDITA3]$Target=[TARGETDIR]
		...
		SetFilePermission(path+"MTConnectAgentFwding.exe") ;
		...
		WritePrivateProfileString("MTCONNECT", "fwdport", agentport.c_str(), (path+"Config.ini").c_str());
		WritePrivateProfileString("MTCONNECT", "servicename", servicename.c_str(), (path+"Config.ini").c_str());
		WritePrivateProfileString("MTCONNECT", "backurl", backendurl.c_str(), (path+"Config.ini").c_str());

		//RunSynchronousProcess(path+"MTConnectAgentFwding.exe", " install");
...
extern "C" UINT __stdcall Uninstall (MSIHANDLE hInstall)
{
	...
	try{
		...
		//::Sleep(3000);
		RunSynchronousProcess("sc.exe", " delete MTConnectAgentFwding");
	}
	catch(...)
	{
