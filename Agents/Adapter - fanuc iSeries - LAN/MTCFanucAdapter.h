...
public:
	...
	HRESULT ErrMsg (const char * format, ...);
	HRESULT DebugMsg (int level, const char * format, ...);
...
//		strncpy(item->_event.mName,tag.c_str(),NAME_LEN);
	...
	void SetMTCTagValue( std::string tag, std::string value)
	{
		...
		if(item==NULL)
		{
			//EventLogger.LogEvent(StrFormat("SetMTCTagValue tag %s not found\n", tag.c_str()));
			GLogger.Warning(StrFormat("SetMTCTagValue tag %s not found\n", tag.c_str()));
			return;
		}
	...
	{	
		...
		GLogger.Warning(StrFormat("GetMTCTagValue tag %s not found\n", tag.c_str()));
		return "";
	}
	int mPort;
	int mSleep;
