...
void MTConnectService::install(int argc, const char *argv[])
{
	...
	{
		Trace("Cannot install service (%d)\n", GetLastError());
		return;
	}
	...
	if (schService != NULL) {
		...
		{
			Trace("ChangeServiceConfig failed (%d)\n", GetLastError());
		}
		else Trace("Service updated successfully.\n"); 
	} else {
		...
		if (schService == NULL) 
		{
			Trace("CreateService failed (%d)\n", GetLastError()); 
			CloseServiceHandle(schSCManager);
			return;
		}
		else Trace("Service installed successfully\n"); 
	}
	...
	{
		Trace("Could not open software key: %d\n", res);
		return;
	}
	...
	if (res != ERROR_SUCCESS)
	{
		//Trace("Could not open MTConnect, creating: %d\n", res);
		res = RegCreateKey(software, "MTConnect", &mtc);
		if (res != ERROR_SUCCESS)
		{
			RegCloseKey(software);
			Trace("Could not create MTConnect: %d\n", res);
			return;
		}
	}
	RegCloseKey(software);
	...
	// Create Service Key
	HKEY adapter;
	Trace("Create Reg key %s");
	Trace(mName);
	res = RegOpenKey(mtc, mName, &adapter);
	if (res != ERROR_SUCCESS)
	{
		//Trace("Could not open %s, creating: %d\n", mName, res);
		res = RegCreateKey(mtc, mName, &adapter);
		if (res != ERROR_SUCCESS)
		{
			RegCloseKey(mtc);
			Trace("Could not create %s: %d\n", mName, res);
			return;
		}
	}
	RegCloseKey(mtc);
...
void MTConnectService::remove()
{
	...
	if (manager == NULL) {
		Trace("Could not open Service Control Manager");
		return;
	}
	service = ::OpenService(manager, mName, SERVICE_ALL_ACCESS);
	CloseServiceHandle(manager);
	if (service == NULL) {
		Trace("Could not open Service ");
		return;
	}

	if(::DeleteService(service) == 0) {
		Trace("Could delete service %s\n", mName);
	} else {
		Trace("Successfully removed service %s\n", mName);
	}
...
VOID SvcInit( DWORD dwArgc, LPTSTR *lpszArgv)
{
...
#if 0
	char key[1024];
	snTrace(key, 1022, "SOFTWARE\\MTConnect\\%s", gService->name());
	key[1023] = '\0';
	Trace(key.c_str());
	...
	if (res != ERROR_SUCCESS)
	{
		Trace("RegOpenKey: Could not open Adapter");
		SvcReportEvent("RegOpenKey: Could not open Adapter");
		ReportSvcStatus( SERVICE_STOPPED, 1, 0 );
		return;
	}

	Trace("SvcInit RegQueryValueEx \n");
	BYTE arguments[2048];
	DWORD len = 2047, type;
	...
	Trace("SvcInit gService->initialize \n");
