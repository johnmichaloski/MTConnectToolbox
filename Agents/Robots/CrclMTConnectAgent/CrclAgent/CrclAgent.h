...
public:
	...
	std::map<std::string, std::vector<std::string> >   mTagnames;    //!< map of tagnames per device
    std::map<std::string, std::vector<std::string> > mConditionTags;   //!< map of condition ids per device
...
//		EventLogger.LogEvent(err);
//	    ExitProcess(-1); 
		// 	std::string cmd = StrFormat("cmd /c net stop \"%s\" ",  adapter->name().c_str());        
...
{
public:
	std::vector<CrclAdapter * >						 _cncHandlers;
...
//	virtual HRESULT	WriteDevicesFile(std::vector<std::string> devicenames,   std::string xmlFile, std::string destFolder);
