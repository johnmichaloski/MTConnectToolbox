...
std::string CProductVersion::GenerateVersionTable(std::string versionstr)
{
	...
	html+="      <body><h1>Version Information </h1><table border=\"1\"><tr> <th>Version</th></tr>\n";
...
std::string CProductVersion::GetProductInstallVersion(std::string configfilename) 
{ 
	std::string contents;
...
{ 
	switch(n)
	{
	case  1800:  return TEXT("Visual Studio 2013 - MSVC++ 12.0 "); 
		case  1700: return TEXT("Visual Studio 2012 - MSVC++ 11.0"); 
		case  1600: return TEXT("Visual Studio 2010 - MSVC++ 10.0");
		case  1500: return TEXT("Visual Studio 2008 - MSVC++ 9.0");
		case  1400: return TEXT("Visual Studio 2005 - MSVC++ 8.0");
		case  1300: return TEXT("Visual Studio 2003 - MSVC++ 7.0");
		case  1200: return TEXT("MSVC++ 6.0");
		case  1100: return TEXT("MSVC++ 5.0");
	};
...
BOOL CProductVersion::IsWow64()
{
    ...
    if(NULL != fnIsWow64Process)
    {
		...
		*/
        ...
        {
            OutputDebugString("CProductVersion::IsWow64() Error:  fnIsWow64Process(GetCurrentProcess(),&bIsWow64))\n");
        }
		else
		{
			 bIsWow64 = TRUE;
...
std::string CProductVersion::GenerateVersionDocument()
{
 ...
 //   s << "<HTML>\n";
	...
	//s << "</TABLE> \n";
	s << "<P align=\"center\" valign=\"middle\" style=\"font-size: 200%\"> MTConnect Agent <BR>VERSION DESCRIPTION DOCUMENT </BR> </P> \n";
	...
	s << "<TR><TD>MTConnect Agent</TD><TD>MTConnect Institute<BR> Mods NIST</BR></TD>";
    s<< StrFormat("<TD>%d.%d.%d.%d</TD></TR>\n", major, minor, revision, build);
	s << "</TABLE>\n";
   ...
   s << "<P>";
	...
	s << "<OL>\n";
		...
		s << "</LI>\n ";
		s << "<LI> The first screen that will appear is a Welcome page. Click Next. At any time you can stop the install by clicking the Cancel button.  \n";
		s << "</LI>\n ";
		s << "<LI> The second screen is the License Agreement page. This outlines the terms of agreement for using the software.\n" ;
		...
		s << "</LI>\n ";
		s << "<LI> The third screen is the Installation Directory page. The user is prompted to select a installation directory. \n";
		...
		s << "</LI>\n ";
		s << "<LI> After the user clicks Next, the fourth page details the installation of all the file is done, but has not been configured. \n";
		s <<" Again, the installation program determines whether to use a 32 or 64 bit installation exe. \n";
		s << "</LI>\n ";
		s << "<LI> The  fifth page prompts the user to configure the Agent specific parameters. \n";
		...
		s << "</LI>\n ";
		s << "<LI> A flash screen alerts the user that the Adapter Wizard is going to run. \n";
		...
		s << " Advanced users should read https://github.com/mtconnect/adapter/blob/master/MTC_SHDR.docx for information on the SHDR protocol. Note, SHDR is not an MTConnect standard, but is widely used as a back-end for device communication. \n";
			s << "<OL type=\"a\">\n ";
			s << "<LI> A Welcome page offers the user a Adapter configuration greeting. Clicking Next is all that is needed. \n";
			s << "</LI>\n ";
			s << "<LI> The second Adapter configuration page, queries the user to select a vendor, machine type and version. \n";
			...
			s << "</LI>\n ";
			s << "<LI> The next Adapter configuration screen asks the necessary information  to configure an Adapter: Device Name, Type, IP and Port. \n";
		...
		s << "</LI>\n";
	s << "</OL>\n";
	s << "<P>There are several steps to insure that the Agent installed correctly. </P>\n";
	
	s << "<OL>\n";
		s << "<LI> First, you will need to double check that the MTConnectAgent installed correctly in the Windows Service Manager (SCM). \n";
		...
		s << "In the popup, you should be on 'General' tab and the see the following: \n";
			...
			s << "<LI>Path to executable as (the default assuming you didnt change the install directory) C:\\Program Files\\MTConnect\\MTConnectAgent\\agent.exe </LI> \n";
			s << "<LI> Service Status should be stopped. If you had checked the service startup it should say started. </LI> \n";
	...
	s << "or <LI>navigate to the install directory (default C:\\Program Files\\MTConnect\\MTConnectAgent) and click uninstall.exe. </LI>\n";
	s << "</OL>\n";
	s << "<P>The control panel uninstall procedure wil be described (although once the uninstall program is activated, the procedure is the same.) \n";
	s << "</P>\n";
...
// From http://forum.codecall.net/topic/60159-how-to-determine-windows-operating-system-version-information-using-visual-c/
...
std::string CProductVersion::GetOSDisplayString()
{
        ...
        osvi.dwMajorVersion > 4 )
   {
      ...
      if ( osvi.dwMajorVersion == 6 )
      {
         if( osvi.dwMinorVersion == 0 )
         {
            ...
            else pszOS+= TEXT("Windows Server 2008 " );
         }
         ...
         if ( osvi.dwMinorVersion == 1 )
         {
            ...
            else pszOS+= TEXT("Windows Server 2008 R2 " );
         }
      ...
      if ( osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 2 )
      {
         ...
         else pszOS+=TEXT("Windows Server 2003, ");
         ...
         if ( osvi.wProductType != VER_NT_WORKSTATION )
         {
            ...
            else if ( si.wProcessorArchitecture==PROCESSOR_ARCHITECTURE_AMD64 )
            {
                ...
                else pszOS+= TEXT( "Standard x64 Edition" );
            }
            ...
            else
            {
                ...
                else pszOS+= TEXT( "Standard Edition" );
            }
         }
      }
      ...
      pszOS+=StrFormat( " (build %d)", osvi.dwBuildNumber);
   ...
   {  
      OutputDebugString( "This sample does not support this version of Windows.\n");
      return pszOS;
   }
}