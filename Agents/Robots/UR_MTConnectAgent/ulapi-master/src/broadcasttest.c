/*!
  \file broadcasttest.c

  \brief Tests send and receive broadcast sockets.
*/
...
int main(int argc, char *argv[])
{
  ...
  if (is_client) {
    fd = ulapi_socket_get_broadcastee_id_on_interface(port, intfptr);
    if (fd <= 0) {
      fprintf(stderr, "can't open broadcastee socket\n");
      return 1;
    }
    while (1) {
    ...
    fd = ulapi_socket_get_broadcaster_id_on_interface(port, intfptr);
    if (fd <= 0) {
      fprintf(stderr, "can't open broadcaster socket\n");
      return 1;
    }
    while (! feof(stdin)) {
      memset(buffer, 0, sizeof(buffer));
