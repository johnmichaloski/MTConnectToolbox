...
int MTConnectService::main (int argc, const char *argv[])
{
    ...
    try
    {
        ...
        OutputDebugString("Enter service routine\n");
        mIsService = true;
...
void MTConnectService::install ( )
{
    ...
    {
        Trace("Cannot install service GetModuleFileName Failed (%x)\n", GetLastError( ));
    ...
    if ( service != NULL )
    {
        ...
        {
            Trace("ChangeServiceConfig Failed (%x)\n", GetLastError( ));
    ...
    else
    {
        ...
        if ( service == NULL )
        {
            Trace("CreateService Failed %s Error= (%x)\n", mName.c_str( ), GetLastError( ));
    ...
    if ( ( mConfigFile[0] != '/' ) && ( mConfigFile[0] != '\\' ) && ( mConfigFile[1] != ':' ) )
    {
        // Relative file name
        // char path[MAX_PATH];
        // GetCurrentDirectory(MAX_PATH, path);// not necessarily in current directory, alway in exe path
    ...
    RegCloseKey(agent);
#endif
    Trace("Service installed successfully\n");
...
void MTConnectService::remove ( )
{
    ...
    if ( manager == NULL )
    {
        Trace("Remove - Could not open Service Control Manager\n");
...
{
    OutputDebugString("Enter SvcMain routine\n");
    ...
    if ( !GetModuleFileName(NULL, path, MAX_PATH) )
    {
        Trace("SvcMain - Cannot get path of executable\n");
...
VOID SvcInit (DWORD dwArgc, LPTSTR *lpszArgv)
{
    ...
    if ( !GetModuleFileName(NULL, path, MAX_PATH) )
    {
        Trace("SvcInit - Cannot get path of executable\n");
        sLogger << dlib::LERROR << "Cannot get path of executable (" << GetLastError( ) << ")";
        return;
    }
    ...
    res = RegOpenKeyEx(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &agent);

    if ( res != ERROR_SUCCESS )
    {
        OutputDebugString("RegOpenKey: Could not open MTConnect Agent Key\n");
    ...
    if ( res != ERROR_SUCCESS )
    {
        OutputDebugString("RegOpenKey: Could not open ConfigurationFile\n");
...
#endif
    const char *argp[2];
    argp[0] = ( std::string(path) + "\\Agent.cfg" ).c_str( );
    argp[1] = 0;
    gService->initialize(1, argp);
