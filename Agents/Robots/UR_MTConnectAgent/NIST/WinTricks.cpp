...
namespace WinTricks
{
    ...
    tstring FormatError (TCHAR *pMessage, DWORD eNum)
    {
        ...
        // return the message
        return StdStringFormat("\n  WARNING: %s failed with error %d (%s)", pMessage,
                               eNum, sysMsg);
    }
/**
Impersonation woks only for current thread.
with local resources it will work as LocalSystem, with the added share it will
work as user on remote computer specified in WNetAddConenction2 (in this case -
Administrator on SomeComputer).
You can omit using drive letter in NETRESOURCE and access files via
"\server\share\filename.ext" notation
*/
    bool LogonAddNetworkResource (tstring remotename, tstring localname, tstring pw,
                                  tstring user)
    {
        HANDLE hToken;

        LogonUser("NETWORK SERVICE", "NT AUTHORITY", NULL,
                  LOGON32_LOGON_NEW_CREDENTIALS, LOGON32_PROVIDER_WINNT50, &hToken);
        ImpersonateLoggedOnUser(hToken);
        NETRESOURCE
            nr;                                            // https://msdn.microsoft.com/en-us/library/windows/desktop/aa385353(v=vs.85).aspx
...
// http://stackoverflow.com/questions/2968426/wnetaddconnection2-from-a-windows-service
    bool AddNetworkResource (tstring remotename, tstring localname, tstring pw,
                             tstring user)
    {
        NETRESOURCE nr;
        ...
        nr.lpRemoteName  = (LPTSTR) remotename.c_str( );   // "\\\\SomeCopmuter\\C$";
        nr.lpLocalName   = (LPTSTR) localname.c_str( );    // "Z:";
        DWORD dwError = WNetAddConnection2(&nr, pw.c_str( ), user.c_str( ),
                                           0);             //  "SomeComputer\\Username"
    ...
    bool NetworkResourceExists (tstring remotename, tstring localname, tstring pw,
                                tstring user)
    {
        NETRESOURCE nr;
    ...
    DWORD MapNetworkDrive (tstring strRemoteName, tstring strLocalDrive,
                           tstring strUseName, tstring strPassword)
    {
        DWORD       dwLastError;
        NETRESOURCE nr;
        ...
        {
            dwLastError
                = WNetAddConnection2(&nr, strPassword.c_str( ), strUseName.c_str( ), 0);
...
' (e.g. a diskette isn't in drive A:)*/
    ...
    void SetFilePermission (tstring szFilename)
    {
        ...
        a = AllocateAndInitializeSid(&SIDAuthWorld, 1, SECURITY_WORLD_RID, 0, 0, 0, 0,
                                     0, 0, 0, &pEveryoneSID);
        ...
        //       SetNamedSecurityInfo("C:\\Program
        //       Files\\Mydir\\My.log",SE_FILE_OBJECT,DACL_SECURITY_INFORMATION,pSD,NULL,NULL,NULL);
        if ( d == 0 )
        {
...
// https://code.msdn.microsoft.com/windowsdesktop/CppCreateLowIntegrityProces-8dd7731d
    BOOL GetProcessIntegrityLevel (PDWORD pdwIntegrityLevel)
    {
        ...
        if ( !GetTokenInformation(hToken, TokenIntegrityLevel, pTokenIL, cbTokenIL,
                                  &cbTokenIL) )
        {
            dwError = GetLastError( );
            goto Cleanup;
...
*/
    bool SetPrivilege (LPCTSTR privilege, bool enablePriv)
    {
        ...
        if ( AdjustTokenPrivileges(hToken, FALSE, &tokenPriv, NULL, NULL, NULL) !=
             FALSE )
        {
            CloseHandle(hToken);
