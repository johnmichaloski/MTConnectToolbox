...
inline std::string UrlEncode (const std::string & input)
{
    ...
    for ( std::string::const_iterator i = input.begin( ); i != input.end( ); ++i )
    {
        ...
        {
            ssOut << '%' << std::setw(2) << std::setfill('0')
...
//	std::string trim=" \t\r\n";
//
//    std::size_t first = str.find_first_not_of(trim);
//    if(first == std::string::npos)
//	{
//		str.clear();
//        return ;
//	}
//	std::size_t last  = str.find_last_not_of(trim);
...
//  }
// inline std::string ChangeExtension( const std::string& path, const
// std::string& ext )
//  {
...
inline std::string nowtimestamp ( )
{
    ...
    GetSystemTime(&st);
    return StdStringFormat("%4d-%02d-%02dT%02d:%02d:%02d.%04d", st.wYear,
...
inline std::map<std::string, std::string> ParseIniSection (std::string filename,
                                                           std::string section)
{
...
inline std::string GetIniProfileString (std::string filename,
                                        std::string section, std::string key)
{
    std::map<std::string, std::string> symbols
        = ParseIniSection(filename, section);
    return symbols[key];
}
template<typename T>
...
inline std::string HrMinSecFormat (std::string sec)
{
    ...
    return StdStringFormat("%02d:%02d:%02d", nsec / 3600, ( nsec % 3600 ) / 60,
                           nsec % 60);
}
...
// From:
// http://www.ccp4.ac.uk/ccp4bin/viewcvs/pointless/string_util.cpp?rev=1.1&content-type=text/vnd.viewcvs-markup
...
inline double POSMAX (double x, double y, double dMin = 0)
{
    x = MAX(x, dMin);
    y = MAX(y, dMin);
...
inline double POSMIN (double x, double y, double dMin = 0)
{
    x = MAX(x, dMin);
    y = MAX(y, dMin);
...
template<typename T>
inline std::vector<T> TokenList (const std::string & str,
                                 const std::string & delimiters)
{
    std::vector<T>         tokens;
    std::string::size_type delimPos = 0, tokenPos = 0, pos = 0;
...
//	{
//		if((strcmp(fname->d_name, ".") != 0) && (strcmp(fname->d_name,
// "..") != 0))
//		{
//			if(fname->d_type == DT_DIR)
//			{
//				filename = path +  fname->d_name;
//				std::string fullpath = RecursiveFileFind(filename,
// filetitle);
//				if(!fullpath.empty())
//					return fullpath;
//			}
...
inline std::string ReplaceOnce (std::string         result,
...
inline bool ReplacePattern (std::string & inStr, std::string oldBeginningStr,
                            std::string oldEndingStr, std::string newStr)
{
    size_t i, j, k;
