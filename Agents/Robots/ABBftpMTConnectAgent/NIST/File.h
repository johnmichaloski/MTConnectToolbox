//
// C:\Users\michalos\Documents\GitHub\Agents\ABBftpMTConnectAgent\NIST\File.h
//
...
public:
	...
	std::string TempFileName(const TCHAR * prefix="TEMP", UINT uniquenum=0)
	{
		TCHAR szTempFileName[MAX_PATH];
		std::string filename;
		//  Generates a temporary file name. 
		DWORD uRetVal = GetTempFileName(TempDirectory().c_str(), // directory for tmp files
			prefix,     // temp file name prefix 
			uniquenum,                // create unique name 
			szTempFileName);  // buffer for name 
		if (uRetVal == 0)
			filename= TempDirectory()+"tmp.txt";
		else
