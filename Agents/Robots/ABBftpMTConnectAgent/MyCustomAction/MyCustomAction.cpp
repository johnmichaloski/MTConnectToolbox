	...
	try {
		...
		for(size_t i=0 ; i< symbols.size() ; i++)
		{
			...
			if(tokens[0]=="AgentPort") // http://agent:portno
				agentport=tokens[1];
			if(tokens[0]=="ServiceName") // default ABBAgent
		...
		SetFilePermission(path+"ABBAgent.exe") ;
		...
		for(size_t i=0; i< keys.size(); i++)
		{
			...
			*/
			WritePrivateProfileString(keys[i].c_str(), "User",  "", (path+"Config.ini").c_str());
			WritePrivateProfileString(keys[i].c_str(), "Pw",  "", (path+"Config.ini").c_str());
			WritePrivateProfileString(keys[i].c_str(), "LocalShare",  "", (path+"Config.ini").c_str());
			WritePrivateProfileString(keys[i].c_str(), "NetworkShare", "", (path+"Config.ini").c_str());

		}
		DbgOut("Create service with ABBAgent.exe");
		...
		//RunSynchronousProcess(path+"ABBAdapter.exe", " install");
...
extern "C" UINT __stdcall Uninstall (MSIHANDLE hInstall)
{
	...
	*/
	...
	try{
		std::string str;
		//RunSynchronousProcess("sc.exe", " stop ABBAdapter");
		//::Sleep(3000);
		RunSynchronousProcess("sc.exe", " delete ABBAdapter");
	}
	catch(...)
	{
