...
class ABBAdapter : public AdapterT
{
public:
  ABBAdapter(AgentConfigurationEx *mtcagent,        // mtconnect agent
    NIST::Config &                    _config,
...
#define SIMPLE
#ifdef SIMPLE
   //std::string lastX,lastY, lastZ;
   double lastX,lastY, lastZ;
#else
  const static size_t lastsize=5;
  std::vector<std::string> lastX,lastY, lastZ;
#endif
  void UpdateTags();
  void Run();
  void Paused();
