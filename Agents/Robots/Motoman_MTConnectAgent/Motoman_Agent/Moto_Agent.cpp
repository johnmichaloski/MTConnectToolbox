...
// Moto_Agent.cpp : Defines the entry point for the console application.
...
void AgentConfigurationEx::start ( )
{
    ...
    {
        Moto_Adapter *_cmdHandler = new Moto_Adapter(this, mConfig, mDevices[i]);
        mAdapterHandlers.push_back(_cmdHandler);
        mGroup.create_thread(
            boost::bind(&Moto_Adapter::doCycle, mAdapterHandlers[i]));
    }
...
// SCM reset command of this service
        ...
        // This works when you run in from a command console as moto_Agent.exe debug,
