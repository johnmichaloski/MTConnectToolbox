...
CAgentCfg::CAgentCfg(void) { Clear( ); }
...
void CAgentCfg::AddDevice (std::string cfgname, std::string devicename,
...
void CAgentCfg::UpdateDevice (std::string cfgname, std::string ip,
                              std::string port)
{
    std::vector<std::string>::iterator it
        = std::find(names.begin( ), names.end( ), cfgname);
...
LRESULT CAgentCfg::WriteCfgFile (std::string cfgfile, std::string xmlFile,
...
LRESULT CAgentCfg::WriteDevicesFile (std::string xmlFile,
                                     std::string destFolder)
{
    std::string contents;
    ...
    for ( UINT i = 0; i < devices.size( ); i++ )
    {
        ...
        config
                  = ReplaceOnce(config, "name=\"NNNNNN\"", "name=\"" + names[i] + "\"");
        contents += config + "\n";
    }
    contents += "</Devices>\n";
    contents += "</MTConnectDevices>\n";
...
LRESULT CAgentCfg::WriteAgentCfgFile (std::string cfgfile, std::string xmlFile,
                                      std::string destFolder)
{
...
{
    CFileDialog dlg(FALSE, _T("*"), ( ::ExeDirectory( ) + "Agent.cfg" ).c_str( ),
    ...
    WriteCfgFile(ExtractFilename(dlg.m_szFileName), "",
                 ExtractDirectory(dlg.m_szFileName));
}
