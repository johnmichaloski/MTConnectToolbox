...
/**
* \brief Tries to determine if an existing MTConnect agent (same name and same
http port) exists.
* Uses windows psapi lib function (difference version 1 & 2) to determine
processes and corresponding process  exe image.
* Given matching process name to current, look up image folder, read agent.cfg
(assuming this is it!) and see what http port it is using.
* If two exe's exist with same name and same http port - Notify user of
potential problem.
* Example:
<PRE>
...
singleton.ExtractDirectory(filepaths[i]);
                        ...
                        {
                                GLogger.Fatal("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                                GLogger.Fatal("Conflict with another MTConnect
agent with same exe name and http port number\n");
                                GLogger.Fatal("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                        }
...
struct MTConnectSingleton
{
  ...
  */
    ...
    * \param filepaths is a vector containing the path to exe matching the
    * processname.
    * \return vector of pids of executables that match processname.
    */
    std::vector<DWORD> GetModules (std::string                processname,
                                   std::vector<std::string> & filepaths)
    {
        ...
        for ( unsigned int i = 0; i < cProcesses; i++ )
        {
            if ( proc_id[i] != 0 )                         // if the id is not empty
            {
                TCHAR proc_name[MAX_PATH]
                    = TEXT("<unknown>");                   // array for storing name of process
                TCHAR file_name[MAX_PATH]
                    = TEXT("<unknown>");                   // array for executable of process
                TCHAR szProcessName[MAX_PATH] = TEXT("<unknown>");

                HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, false,
                                              proc_id[i]); // open the process
                ...
                if ( EnumProcessModules(hProcess, &hMod, sizeof( hMod ), &cbNeeded) )
                {
                    ...
                    QueryFullProcessImageName(
                        hProcess, 0, file_name,
                        &size);                            // Get the name of the image base file
