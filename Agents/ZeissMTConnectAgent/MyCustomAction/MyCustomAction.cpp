	...
	try {
		...
		for(size_t i=0 ; i< symbols.size() ; i++)
		{
			...
			if(tokens[0]=="AgentPort") // http://agent:portno
				agentport=tokens[1];
			if(tokens[0]=="ServiceName") // default ZeissAgent
		...
		SetFilePermission(path+"ZeissAgent.exe") ;
		...
		DbgOut("Create service with ZeissAgent.exe");
		...
		//RunSynchronousProcess(path+"ZeissAdapter.exe", " install");
...
extern "C" UINT __stdcall Uninstall (MSIHANDLE hInstall)
{
	...
	*/
	...
	try{
		std::string str;
		MessageBox(NULL, "Uninstall ZeissAgent", "0", MB_OK);
		RunSynchronousProcess(path+"ZeissAgent.exe", " remove ZeissAgent");
	}
	catch(...)
	{
