...
/***** METHODS *****/
extern std::string int64ToString(uint64_t i);
...
extern std::string getCurrentTime(time_t aTime, int aUsec, TimeFormat format);

/* Get the current time formatted */
extern std::string getCurrentTime(TimeFormat format);

/* time_t to the ms */
extern uint64_t getCurrentTimeInMicros();

/* Get the relative time from using an uint64 offset in ms to time_t as a web time */
extern std::string getRelativeTimeString(uint64_t aTime);

/* Get the current time in number of seconds as an integer */
extern unsigned int getCurrentTimeInSec();

extern uint64_t parseTimeMicro(const std::string &aTime);

/* Replace illegal XML characters with the correct corresponding characters */
extern void replaceIllegalCharacters(std::string& data);

/* Return enumeration values according to a string name and array */
extern int getEnumeration(
