...
namespace NIST
{
	...
	*/
	...
	public:
		...
		template<typename Value>
		std::map<std::string, Value> getvaluemap(std::string section) 
		{ 
			...
			for(int i=0; i< keys.size(); i++)
			{ 
				mapping[keys[i]] = GetSymbolValue(section + "." + keys[i]).value<Value>();
			}
			return mapping; 
		}
		...
		{ 
			...
			for(int i=0; i< keys.size(); i++)
			{ 
				std::string value = GetSymbolValue(section + "." + keys[i]).c_str();
