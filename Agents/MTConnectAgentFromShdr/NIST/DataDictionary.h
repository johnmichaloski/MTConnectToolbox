...
public:
	...
	std::string & operator[](const std::string& cstr)
	{
		std::string str=cstr;
		std::transform(str.begin(),str.end(),str.begin(),tolower);
		DataDictionary::iterator it = this->find(str);
		if(it==this->end()) 
	...
	void AddPairs(std::vector<std::string> items, std::vector<std::string> values)
	{
		for(int i=0; i < items.size(); i++)
		{
			this->operator [](items[i])=values[i];
		}
	}
	template<typename T>
