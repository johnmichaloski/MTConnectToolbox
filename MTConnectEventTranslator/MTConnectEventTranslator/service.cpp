...
{
 	AtlTrace("MTConnectService::initialize(int aArgc, const char *aArgv[])");
...
{ 
  ...
  // Otherwise, the service is probably being started by the SCM.
 	AtlTrace("MTConnectService::main(int argc, const char *argv[]) ");
...
{
	GLogger.Fatal("SvcMain( DWORD dwArgc, LPTSTR *lpszArgv )\n" ); 
 // Register the handler function for the service
  ...
  gSvcStatusHandle = RegisterServiceCtrlHandlerEx( 
    gService->name(), 
    (LPHANDLER_FUNCTION_EX) SvcCtrlHandlerEx,
	NULL);
...
//   None
//
DWORD SvcInit( DWORD dwArgc, LPTSTR *lpszArgv)
{
...
//   None
//
DWORD WINAPI SvcCtrlHandlerEx( DWORD dwCtrl,
						   DWORD dwEventType, 
                       LPVOID lpEventData, 
                       LPVOID lpContext)
{
  // Handle the requested control code. 
	GLogger << FATAL << "SvcCtrlHandlerEx" << std::endl; 
...
/*DWORD WINAPI HandlerEx(DWORD dwControl,
                       ...
                       LPVOID lpContext)
{
	GLogger << FATAL << "HandlerEx" << std::endl; 
	switch (dwControl)
	{
	case (SERVICE_CONTROL_SESSIONCHANGE):
		switch(dwEventType)
