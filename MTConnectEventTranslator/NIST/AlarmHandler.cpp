...
#include "SimMath.h"
#include "Registry.h"
static char * AppRegistryKey = "Software\\NIST\\MTConnectEventTranslator";
...
{
	_faultitems = "Timestamp,Machine,Shift,Description,Program,RaisedAt,ResolvedAt,MTBF, MTTR";
	_tbftimer.restart();
}
	...
	for(int i=0; i< _faults.size(); i++)
	{
		...
		{	
			...
			if(_currentfault != _lastfault)
			{
				...
				_alarmdatum.clear();
				_alarmdatum["RaisedAt"] = GetTimeStamp(LOCAL); // datum["Timestamp"];
				...
				_alarmdatum["ResolvedAt"] = GetTimeStamp(LOCAL); // datum["Timestamp"];
