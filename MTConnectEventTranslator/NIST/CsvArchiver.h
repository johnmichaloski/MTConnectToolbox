...
Example:
	ADODataBase db;
	db.OpenExcel("D:\\Precision Sand Casting Data.xls", NULL, NULL);
	ADODB::_RecordsetPtr rec= db.Execute("SELECT AverageCycleTime FROM [Finish Line 1$] WHERE MachineName='-Core Dump 1'");
	db.DumpRecordset(rec);
*/

class ADODataBase
{
public:
	std::string DumpError(_com_error &e);
	void OpenExcel(LPCTSTR CnnStr, LPCTSTR UserName, LPCTSTR Pwd);
	ADODB::_RecordsetPtr Execute(LPCTSTR sqlquery);
	std::string DumpRecordset(ADODB::_RecordsetPtr pRecordset, bool bValueOnly=TRUE);
	void AddRecord(ADODB::_RecordsetPtr pRecordset, _variant_t FieldValues[], short iFieldCount );
	void CreateJetDb(LPCTSTR filename, LPCTSTR UserName, LPCTSTR Pwd);
	void OpenJetDb(LPCTSTR filename, LPCTSTR UserName, LPCTSTR Pwd);
	/////////////
...
public:
	...
	int main()
	{
		...
		read("data.xls", 0, true, "data.csv");   // reading a first sheet with header from data.xls
		read("test.xls", 0, false, "test.csv");  // reading a first sheet without header from test.xls
	}
};
#endif
