...
//Returns the value ln[??(xx)] for xx > 0. Ch. 6
...
double Distribution::RandomVariable()
{
	...
	else if(_szDistribution=="probability")
		return rv.probability(_parameters[0]);

	throw _T("No distribution set");
	return 0.0;
} 

bool Distribution::IsDistribution(std::string str)
{
	std::string szDistribution;
	std::vector<std::string> tokens;
	tokens = TrimmedTokenize(str, ",");
	if(tokens.size()>0) szDistribution=tokens[0].c_str();
	if(szDistribution.empty())
		return false;
	...
	if(szDistribution=="constant")
		...
		return true;
	else if(szDistribution=="probability")
		return true;
	return false;
} 
...
double Distribution::pdf(double x, LPCTSTR lpstrDistribution, std::vector<double> parameters )
{
	...
	{
		boost::math::geometric g(parameters[0]);
		return boost::math::pdf(g, x);
	}	
