...
public:
	...
	bool EstimateTruth(double percentage)
	{
...
public:
	...
	void SetParameters(std::string str)
	{
		std::vector<std::string> tokens;
		tokens = TrimmedTokenize(str, ",");
		if(tokens.size()>0) _szDistribution=tokens[0].c_str();
		if(tokens.size()>1) _parameters[0]=ConvertString<double>(tokens[1],0);
		if(tokens.size()>2) _parameters[1]=ConvertString<double>(tokens[2],0);
		if(tokens.size()>3) _parameters[2]=ConvertString<double>(tokens[3],0);
		if(tokens.size()>4) _parameters[3]=ConvertString<double>(tokens[4],0);
	}
	static bool IsDistribution(std::string str);
