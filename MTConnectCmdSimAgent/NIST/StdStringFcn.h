...
template<typename T,  std::string T::*Var>
inline std::string StringFromVectorVar(std::vector<T *> c)
{
	std::string tmp;
...
template<typename T>
inline std::string StringFromVector(T t)
{
	std::string tmp;
...
inline std::string StringFromVector(std::vector<bstr_t> t)
{
	std::string tmp;
	for(int i=0; i< t.size(); i++)
	{
...
{
	std::string a = MakeUpper(strHaystack);
	std::string b= MakeUpper(strNeedle);
