...
struct Stats
{
	Stats()
	{	 
		nTotalParts= nGoodParts=nTimes=0;
...
typedef boost::tuple<std::string, std::string, std::string, std::string, double , double  > CostFcnTuple;
	...
	public: 
		...
		unsigned int 				UpdateRateMs() { return _updaterate * _dSpeedup;  }
		double						UpdateRateSec() { return _updaterate / 1000.0 * _dSpeedup; }
		void						SetUpdateRate(unsigned int milliseconds) {if(milliseconds>0) _updaterate=milliseconds; }
		...
		static double				GetAllMachineTiming(StateType state) ;
		...
		double									_nMsTimerElapsed ;  // cycle
		int										_nTMsTimeStart;
		static double							_nMsMachinesTimeElapsed ; // all machines
		double									_nMsTimeElapsed ;			// this state machine
	...
	protected:
		...
		static std::map<StateType, double>		_allstateMapTiming;
	...
	__declspec(selectany)   std::map<StateType, double> ControlThread::_allstateMapTiming;

	__declspec(selectany)   double	ControlThread::_nMsMachinesTimeElapsed ;
