...
namespace StateModel
{
	...
	void ControlThread::Loop()
	{
		...
		while(IsActive())
		{
			t.restart();
			this->Cycle();
			int wait_time = UpdateRateMs() - (1000* t.elapsed()); 
			if(wait_time<0)
				wait_time = 1;
			::Sleep(wait_time);
			//if(wait_time > 0)
	...
	void ControlThread::Cycle()
	{
		...
		_nMsMachinesTimeElapsed+=_nMsTimerElapsed*1000.0;
		_nMsTimeElapsed+=_nMsTimerElapsed*1000.0;

		stateTimes[GetState()]+=UpdateRateSec();
		_allstateMapTiming[GetState()]+=UpdateRateSec();
		TraceSave(GetState(),UpdateRateSec());
	...
	void ControlThread::ResetStateTiming()
	{
		for(std::map<StateType, double>::iterator it=_stateMapTiming.begin(); it!=_stateMapTiming.end(); it++)
			(*it).second=0.0;
		for(std::map<StateType, double>::iterator it=_allstateMapTiming.begin(); it!=_allstateMapTiming.end(); it++)
			(*it).second=0.0;

		
	}
	double ControlThread::GetStateTiming(StateType state) { return _stateMapTiming[state]; }
	double ControlThread::GetAllMachineTiming(StateType state) { return _allstateMapTiming[state]; }
