...
void XMLClient::HandleConnect(const boost::system::error_code& error) {
    ...
    if (error == boost::asio::error::connection_refused) {
        Globals::DebugMessage("HandleConnect connect refused\n");
        Globals::Sleep(_nMSServerConnectRetry);
        Connect();
        return;
    } else if (error == boost::asio::error::already_started) {
        Globals::DebugMessage("HandleConnect connect already_started\n");
        Globals::Sleep(_nMSServerConnectRetry);
        Connect();
        return;
    }
...
void XMLClient::wait_callback(const boost::system::error_code& error, socket_ptr _socket) {
    Globals::DebugMessage("XMLClient::wait_callback");

    if (error == boost::asio::error::operation_aborted) {
        // Data was read and this timeout was canceled
		Globals::DebugMessage("Data was read and this timeout was canceled");
        return;
    } else if (error) {
        std::cout << "read_timeout Error - " << error.message() << std::endl;
        return;
    }
	Globals::DebugMessage("will cause read_callback to fire with an timeout error");
...
void XMLClient::TimerReset() {
    ...
    Globals::DebugMessage(Globals::StrFormat("Timer reset %d Error code=0x%x\n", n, ec));
...
void XMLClient::SyncWrite(std::string str) {
    try {
        ...
        //http://theboostcpplibraries.com/boost.exception
        Globals::DebugMessage("SyncWrite Exception\n");
        std::cerr << boost::diagnostic_information(ex);
        Disconnect();
    } catch (...) {
        Globals::DebugMessage("SyncWrite Exception\n");
