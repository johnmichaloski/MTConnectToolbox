...
int Serial::readFully(char *aBuffer, int len, uint32_t timeout)
{
  ...
  while (consumed < len && (getTimestamp() - start) < timeout) {
    ...
    if (res > 0) {
      ...
      if (cnt < 0) {
        gLogger->debug("Read returned: %d", res);
        return -1;
      }
      consumed += cnt;
    } else if (res < 0) {
      gLogger->debug("Wait returned: %d", res);
      return res;
    }
  }
  ...
  while (written < len && (getTimestamp() - start) < timeout) {
    ...
    if (res > 0) {
      ...
      if (cnt > 0) {
        ...
        gLogger->debug("Write returned: %d", res);
        return -1;
      }
    } else if (res < 0) {
      gLogger->debug("Wait returned: %d", res);
