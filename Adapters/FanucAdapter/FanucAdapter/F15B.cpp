...
int CF15B::getInfo()
{
	...
	GLogger.LogMessage(StdStringFormat("CF15B::connect axes number %d", _adapter->_nAxes ), HEAVYDEBUG);

	if(_adapter->_nAxes >= MAX_AXIS)
	{
		GLogger.Warning(StdStringFormat("CF15B::connect axes number %d > MAX_AXIS OF HEADER!!!", _adapter->_nAxes ));
		disconnect();
	}
	...
	if (EW_OK != ret)
	{
		...
		return 	GLogger.Warning(StdStringFormat("CF15B::connect cnc_rdaxisname FAILED %d", ret ));
	}
