...
socket_communication::socket_communication(Socket_Record_Dlg *dlg)
{
    mParent = dlg;
}
...
HRESULT socket_communication::init (std::string host, std::string port, std::string filename)
{
    ...
    {
        filename = Logging::CLogger::ExeDirectory( ) + filename;
    }
    rawSocketFile.open(filename.c_str( ), std::fstream::out, OF_SHARE_DENY_NONE);

    if ( !rawSocketFile )
    {
        logFatal("ERROR, couldn't open %s\n", mFilename.c_str( ));
        return E_FAIL;
    }
    return S_OK;
}
        ...
        try
        {
            ...
            for ( ; mKeepalive; )
            {
                ...
                if ( len > 0 )
                {
                    ...
                    ss << Nist::RawDump((void *) &buf[0], len, "%02X,");
