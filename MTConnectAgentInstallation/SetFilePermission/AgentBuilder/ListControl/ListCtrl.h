...
public:
	...
	{
		// load list images
		if ( !m_ilListItems.CreateFromImage((UINT) IDB_LISTITEMS, 16, 0, RGB( 255, 0, 255 ), IMAGE_BITMAP, LR_CREATEDIBSECTION ) )
			return FALSE;
			...
			case ITEM_FORMAT_ADDBOX:
				{
					...
					m_ilListItems.DrawEx( nCheckValue > 0 ? ITEM_IMAGE_ADDBOX : ITEM_IMAGE_SUBBOX, dcPaint, rcCheckBox, CLR_DEFAULT, CLR_DEFAULT, ILD_TRANSPARENT );
				}
				break;
			case ITEM_FORMAT_PROGRESS:			
				if ( m_bShowThemed && !m_thmProgress.IsThemeNull() && !( nItemFlags & ITEM_FLAGS_PROGRESS_SOLID ) )
				{
					// draw progress frame
...
public:
	...
	CListItem< TData > CreateListItem( LPCTSTR lpszText, int nImage = ITEM_IMAGE_NONE, UINT nFormat = ITEM_FORMAT_NONE, UINT nFlags = ITEM_FLAGS_NONE )
	{
	...
	{
		CListItem< TData > listItem= CreateListItem(lpszText, nImage, nFormat, nFlags );
		return AddItem( listItem );
	}
	int InsertItem(int nIndex, LPCTSTR lpszText, int nImage = ITEM_IMAGE_NONE, UINT nFormat = ITEM_FORMAT_NONE, UINT nFlags = ITEM_FLAGS_NONE )
	{
		CListItem< TData > listItem= CreateListItem(lpszText, nImage, nFormat, nFlags );
		if ( !m_aItems.InsertAt( nIndex, listItem ) )
			return -1;
		return CListImpl< CListCtrlData >::AddItem() ? nIndex : -1;
	}
	...
	BOOL GetItemComboList( int nItem, int nSubItem, CListArray < CString >& aComboList )
	{
		CSubItem listSubItem;
		// Michaloski addition
		NotifyParent( m_nFocusItem, m_nFocusSubItem, LCN_ENTERCOMBO );
		//////////////////////

		if ( !GetSubItem( m_nFocusItem, m_nFocusSubItem, listSubItem ) )
		//if ( !GetSubItem( nItem, nSubItem, listSubItem ) )
			return FALSE;
