...
inline bool TestPing(std::string host)
{
	...
	if(hIcmp == INVALID_HANDLE_VALUE)
		return 0; // "IcmpCreateFile Failed";
	...
	// returns the number of ICMP_ECHO_REPLY or ICMP_ECHO_REPLY32 structures stored in the ReplyBuffer.
...
#if 1
	delete ReplyBuffer;
	return (dwRetVal != 0) ; // >0 means succceeded
...
inline int TestSocket(std::string url,int port, std::string &errmsg )
{
	int iResult=0;
    WSADATA WSAData;
	...
	std::string error;
	hostent *he;
	if ((he = gethostbyname(url.c_str())) == 0) {
    ...
    iResult=connect(sock, (SOCKADDR *)&sin, sizeof(sin)); // on se connecte sur le site web.
   // send(sock, srequete.c_str(), srequete.size() + 1, 0); // on envoie la requête HTTP.
    //do
    //{
	...
	{
		std::stringstream str("connect function failed with error: %ld\n", WSAGetLastError());
		errmsg=str.str();
	}
...
public:
	...
	CDCOMEnums()
	{
		...
		for(UINT i=0; i< _authenums.size(); i++)
