...
inline std::string Merge(const std::vector<std::string> & values, std::string delimiter=",")
{
	std::string str;
...
/**
	std::string test;
	LoadHtmlResource(test, IDR_HTML1, ::GetModuleHandle(NULL));
*/
inline bool LoadHtmlResource(std::string & contents, UINT nId, HMODULE hModule=NULL)
{
	contents.clear();
	...
	HRSRC hRes				= ::FindResource(hModule, MAKEINTRESOURCE(nId), RT_HTML);
	HGLOBAL hgbl			= ::LoadResource(hModule, hRes);
	if( (hRes == NULL) || (hgbl== NULL))
	{
		AtlTrace(_T("String not loaded 0x%X\n"), ::GetLastError());
		return false;
	}
	DWORD dwSize			= ::SizeofResource(hModule, hRes);
	//char * pLockedResource	= (char *) LockResource(hgbl);
	LPCTSTR lpcHtml = static_cast<LPCTSTR>(LockResource(hgbl));
	CString test( lpcHtml);
	//contents.reserve(static_cast<std::string::size_type>(dwSize));
	//strcpy((char*)contents.c_str(),(const char*) pLockedResource);
	contents=(LPCSTR) test;
	UnlockResource(hgbl);
