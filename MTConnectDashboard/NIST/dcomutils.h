...
__declspec(selectany)  CDCOMEnums DCOMEnums;
	...
	tstringVector _users;
	int _nUser;
	_bstr_t sClsid;
	_bstr_t server;
	tstring sUserSettings;
inline HRESULT Connect(void)
{
	HRESULT hr;
	tstring status = COleDateTime::GetCurrentTime().Format();
	...
	n_impLevelComSecurityEnum=RPC_C_IMP_LEVEL_IMPERSONATE;

	n_authnEnum=RPC_C_AUTHN_WINNT;
	n_authzEnum=RPC_C_AUTHZ_NONE;
	n_authnlevelEnum = RPC_C_AUTHN_LEVEL_CONNECT;
	n_impersonationlevelEnum=RPC_C_IMP_LEVEL_IMPERSONATE;
	...
	COAUTHIDENTITY* pAuthIdentityData; 
	tstringVector info = Tokenize(_users[_nUser], ",");
	if(info.size()<3)
	{
