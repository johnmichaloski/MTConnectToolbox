...
{
public:
    MyShdrServer & _backEnd; /**< socket backend for this parser */
    ...
    */
    ...
    ShdrParser(MyShdrServer & backEnd) : _backEnd(backEnd)
    {
        _bRepeat=false;
        _bFirstTime=true;
	...
	double  ProcessStream()
	{
		...
		if(!timestamp.empty())
		{
			...
			if(_bFirstTime)
			{
                ...
                _duration =  std::chrono::system_clock::duration::zero();
				_bFirstTime=false;
			}
			else
			{
	...
	*/
	void Init(std::string filename) 
	{
        ...
        if ( in.fail() )
        {
            throw std::runtime_error(StrFormat("Bad SHDR file %s",_shdrfilename.c_str(), strerror(errno)));
        }
    }
	/**
...
protected:
    ...
    Timing::MyTime                           _realtimetime; /**< now + elapsed SHDR time */
    Timing::MyTime                           _currenttime; /**< shdr current timestamps */
    Timing::MyTime                           _lasttime; /**< last shdr current timestamps */
    Timing::MyDuration                       _duration; /**< duration between last and current shdr timestamps */
